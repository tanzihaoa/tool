...
object CalendarTimeUtil {
    ...
    fun changeFormatDate(date: Date): String? {
        val dFormat = SimpleDateFormat("yyyy-MM-dd")
        var curDate: String? = null
        try {
            curDate = dFormat.format(date)
        } catch (e: ParseException) {
            e.printStackTrace()
        }
    ...
    fun getWeekStr(dateStr: String): String {
        ...
        val todayStr = dateFormat.format(
...
//            return if (hour > 9) hour.toString() else "0${hour.toString()}"
    ...
    fun getFutureTimeStrings(): MutableList<String> {
        val list = arrayListOf("现在")
        val currentTime = getCurrentTime()
        var splitIndex = 0
        for (i in timeStrings.indices) {
            val hour = timeStrings[i].split(":")[0]
            if (currentTime.startsWith(hour)) {
                splitIndex = i
                break
            }
        }
        for (i in (splitIndex + 1) until timeStrings.size) {
            list.add(timeStrings[i])
        }
