...
class TimeUtil private constructor() {
    ...
    companion object {
        ...
        fun getTomorrowLocalData(): String {
        ...
        fun formatDuration(durationOfSeconds: Long): String {
            val format = "%1$,02d:%2$,02d:%3$,02d"
            val hours = durationOfSeconds / (60 * 60)
            val minutes = durationOfSeconds / 60 - hours * 60
            val seconds = durationOfSeconds - minutes * 60 - hours * 60 * 60
            val array = arrayOf<Any>(hours, minutes, seconds)
            return String.format(format, *array)
        }

        fun formatDurationShort(durationOfSeconds: Long): String {
            val format = "%1$,02d:%2$,02d"
            val hours = durationOfSeconds / (60 * 60)
            val minutes = durationOfSeconds / 60 - hours * 60
            val seconds = durationOfSeconds - minutes * 60 - hours * 60 * 60
            val array = arrayOf<Any>(minutes, seconds)
            return String.format(format, *array)
        }
        ...
        fun getTimeSpan(
            ...
            return millis2TimeSpan(
                string2Millis(time1, format) - string2Millis(time2, format),
                unit
            )
        }
         ...
         * @return
         */
        fun getSecondsBetween(beginDate: Date?, endDate: Date?): Int {
            ...
            return ((endTime - beginTime) / 1000).toInt() //先算出两时间的毫秒数之差
        }
